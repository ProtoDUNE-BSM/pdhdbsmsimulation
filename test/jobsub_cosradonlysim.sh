...
DMWORKDIR=${PWD}
TOTALEVENTS=1000 # total events to generate per wob/flav/decay
NEVENTS=1 # number of events per job - probably want to keep this to 1

FCL="prod_cosmicradonly_protodunehd.fcl"
...
do
        jobsub_submit --group=dune --singularity-image /cvmfs/singularity.opensciencegrid.org/fermilab/fnal-wn-sl7:latest \
          ...
          --resource-provides=usage_model=DEDICATED,OPPORTUNISTIC --expected-lifetime=48h --disk=10GB --memory=10GB --cpu=1 -N 1 \
          --tar_file_name=dropbox://${DMWORKDIR}/LocalProdNumu.Blob.tar.gz \
          file://${DMWORKDIR}/srcs/pdhdbsmsimulation/test/run_fermigridprod_cosradonly_trigger.sh ${FCL} ${NEVENTS} ${NSKIP}
done
